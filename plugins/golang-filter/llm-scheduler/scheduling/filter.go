...
func (f *filter) Filter(req *LLMRequest, pods []*backend.PodMetrics) ([]*backend.PodMetrics, error) {
	// api.LogDebugf("Running filter %q on request %v with %v pods", f.name, req, len(pods))
	...
	if err == nil && len(filtered) > 0 {
		...
		api.LogDebugf("onSuccess %q -> %q, filtered: %v", f.name, next.Name(), len(filtered))
		...
		api.LogDebugf("onFailure %q -> %q, pods: %v", f.name, next.Name(), len(pods))
